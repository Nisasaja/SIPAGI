...
class AdminController extends Controller
{
    ...
    public function index()
    {
       ...
       // Menghitung balita lulus (status_bb_u 'Normal' berdasarkan data terakhir per balita)
        ...
        $pengukuran = Pengukuran::with('profile')->get();
        $dataPerTahun = [];

        // statis beradasarkan desa
     $balitaPerDesaBBU = Pengukuran::select('profile.alamat', 'pengukuran.status_bb_u')
        ->join('profile', 'pengukuran.profile_id', '=', 'profile.id') // Join ke tabel profiles
        ->groupBy('profile.alamat', 'pengukuran.status_bb_u')
        ->get();

     $balitaPerDesaTBU = Pengukuran::select('profile.alamat', 'pengukuran.status_tb_u')
        ->join('profile', 'pengukuran.profile_id', '=', 'profile.id') // Join ke tabel profiles
        ->groupBy('profile.alamat', 'pengukuran.status_tb_u')
        ->get();
